...
%let EM_NUM_VARS = 17;
   %macro EM_VARS;
    ConsumerGroup age balance contacts day days_since default education job
   last_campaign_outcome loan month mortgage phone_type total_contacts val_email y
   %mend EM_VARS;
%let EM_NUM_CLASS = 11;
   %macro EM_CLASS;
    ConsumerGroup default education job last_campaign_outcome loan month mortgage
   phone_type val_email y
   %mend EM_CLASS;
%let EM_NUM_INTERVAL = 6;
   %macro EM_INTERVAL;
    age balance contacts day days_since total_contacts
   %mend EM_INTERVAL;
